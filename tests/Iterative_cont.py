...
def iterative_MI(X, y, **kwargs):
    ...
    for prop, value in list(mi_calc_properties.items()):
                ...
                if (len(conditional_features) > 0):
                    try:
                        if len(conditional_features) == 1:
                            ...
                            candidate_data = candidate_data.tolist() ####################################
                            conditional_data = conditional_data.tolist() ####################################
        ...
        if passed_stats_test:
            ...
            if continue_redundant:
                if len(conditional_features) > 0:
                    ...
                    if verbose:
                        print(
                            'Accepting variable anyway, since we''re forcing redundant selection and are not conditioning on any variables at this round')
                    accepting_feature = True
                ...
                if verbose:
                    print((
                                      'Terminating forward feature selection as we have reached maximum prescribed number of sensors: %d of %d' % (
